...
//    @PostMapping("/login")
    public String loginV1(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        // 쿠키에 시간 정보 추가를 안하면 세션 쿠키 (브라우저 종료 시 만료)
        Cookie idCookie = new Cookie("memberId", String.valueOf(loginMember.getId())); // 쿠키 등록
...
//    @PostMapping("/logout")
    ...
    public String logoutV3(HttpServletRequest request) {
        HttpSession session = request.getSession(false); // 세션이 없더라도 생성을 방지
        if (session != null) {
            session.invalidate(); // 해당 세션과 데이터를 모두 clear
        }
        return "redirect:/";
    }
